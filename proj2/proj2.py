...
def run():
    ...
    print(
        f"  train={accuracy_score(y_tr, lr_gd.predict(X_tr_s)):.3f}  "
        f"test={accuracy_score(y_te, lr_gd.predict(X_te_s)):.3f}")
    ...
    for criterion in ['entropy', 'gini']:
        for depth in [3, 5, 10, None]:
            ...
            dt.fit(X_tr, y_tr)
            print(
                f"  {criterion}, depth={depth}: train={accuracy_score(y_tr, dt.predict(X_tr)):.3f}  "
                f"test={accuracy_score(y_te, dt.predict(X_te)):.3f}")

    print("\n=== Random Forest ===")
    for n in [10, 50, 100, 200]:
        rf = RandomForestClassifier(n_estimators=n, random_state=42, max_depth=4)
        rf.fit(X_tr, y_tr)
        print(
            f"  n={n}: train={accuracy_score(y_tr, rf.predict(X_tr)):.3f}  "
            f"test={accuracy_score(y_te, rf.predict(X_te)):.3f}")

    print("\n=== SVM ===")
    svm = SVC(kernel='rbf', C=1.0, random_state=42, probability=True)
    svm.fit(X_tr_s, y_tr)
    print(
        f"  train={accuracy_score(y_tr, svm.predict(X_tr_s)):.3f}  "
        f"test={accuracy_score(y_te, svm.predict(X_te_s)):.3f}")
    ...
    print(
        f"  hard: train={accuracy_score(y_tr, v_hard.predict(X_tr_s)):.3f}  "
        f"test={accuracy_score(y_te, v_hard.predict(X_te_s)):.3f}")
    print(
        f"  soft: train={accuracy_score(y_tr, v_soft.predict(X_tr_s)):.3f}  "
        f"test={accuracy_score(y_te, v_soft.predict(X_te_s)):.3f}")
